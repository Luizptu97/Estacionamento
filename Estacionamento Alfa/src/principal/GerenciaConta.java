...
public class GerenciaConta {
	...
	public void cadastrarConta() {
		...
		if (veiculos.isEmpty() || clientes.isEmpty() || patios.isEmpty()) {
			...
			do {
				...
				if (nomePatio.equals("0")) {
					...
					while (it.hasNext()) {
						...
						System.out.println(
								"Localizado em: " + p.getBairro() + ", " + p.getMunicipio() + " - " + p.getEstado());
			...
			achou = false;
			for (Conta c : contas) {
				if (c.getPatio().getNome().equalsIgnoreCase(nomePatio)
						&& c.getVeiculo().getPlaca().equalsIgnoreCase(placa)) {
	...
	public void excluirConta() {
		...
		teste = true;
		for (Conta c : contas) {
			if (c.getVeiculo().getPlaca().equalsIgnoreCase(placa) && (c.getCliente().getCodigo() == intTeste)
					&& (c.getMes() == MES) && c.getAno() == ANO) {
				Conta conta = c;
			} else {
		...
		if (teste != false) {
			do {
				...
				if (conta.getDiarias() + diarias > 31) {
					System.out.println(
							"**** O numero de diárias ultrapassou o limite mensal, para isso crie uma nova conta no mês seguinte **** ");
				} else {
